# Limma for co-culture EXMac vs Mac (exposed vs naive macrophages)
library(readxl)
library(limma)
...
for (inp in inputs) {
  ...
  sheet <- inp$sheet
  message("Processing EXMac vs Mac: ", inp$file, " / ", sheet)
  ...
  # Keep only Mac and EXMac columns
  area_cols <- area_cols[grepl("_Mac_", area_cols) | grepl("_EXMac_", area_cols)]
  if (length(area_cols) == 0) {
    message("  No Mac/EXMac columns; skipping")
    next
  }

  mat <- as.matrix(df[, area_cols])
  samples <- data.frame(sample=colnames(mat), stringsAsFactors=FALSE)
  samples$condition <- ifelse(grepl("_EXMac_", samples$sample), "EXMac",
                        ifelse(grepl("_Mac_", samples$sample), "Mac", NA))
  ...
  # detection filter: >=2 detections in EXMac before impute
  ex_detect <- rowSums(!is.na(mat[, samples$condition=="EXMac", drop=FALSE]))
  keep_rows <- ex_detect >= 2
  mat_use <- mat_log2[keep_rows, , drop=FALSE]

  # design
  samples$condition <- factor(samples$condition, levels=c("Mac","EXMac"))
  design <- model.matrix(~0+condition, data=samples)
  colnames(design) <- c("Mac","EXMac")
  ...
  contrast <- makeContrasts(EXMac-Mac, levels=design)
  ...
  out_file <- file.path(out_dir, paste0("limma_", sheet, "_EXMac_vs_Mac_donorblock_LOD_log2.csv"))
  write.csv(out, out_file, row.names=FALSE)

  sig <- sum(out$adj.P.Val < 0.05, na.rm=TRUE)
  up <- sum(out$adj.P.Val < 0.05 & out$logFC > 0, na.rm=TRUE)
  summary_rows[[length(summary_rows)+1]] <- data.frame(
    sheet=sheet, tested=nrow(out), FDR_lt_0_05=sig, up_in_EXMac=up, LOD=LOD,
    detection_filter="EXMac>=2"
  )
}

if (length(summary_rows) > 0) {
  summary_df <- do.call(rbind, summary_rows)
  write.csv(summary_df, file.path(out_dir, "limma_coculture_EXMac_Mac_summary.csv"), row.names=FALSE)
}
