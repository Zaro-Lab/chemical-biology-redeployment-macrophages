...
df <- read_excel(file, sheet = "4A_EatersNon_SE_Light")
cols <- grep("^NArea", names(df), value=TRUE)
mat <- as.matrix(df[, cols])

# sample info
samples <- data.frame(sample=colnames(mat))
samples$condition <- ifelse(grepl("_E_", samples$sample), "E", "NE")
samples$donor <- sub("NArea (RV\\d+)_.*", "\\1", samples$sample)
...
write.csv(out, file.path(out_dir, "limma_4A_light_donorblock_LOD_log2.csv"), row.names=FALSE)
